...
VALUES
    ('Array vs LinkedList', '배열(Array)과 연결 리스트(LinkedList)의 데이터 접근 및 데이터 추가/삭제 관점에서 각각의 특성과 장단점을 비교해 주세요.',
     ...
     '', 1, now(), now()),
    ('객체지향 프로그래밍의 특징', '객체지향 프로그래밍의 대표적인 4가지 특징이 무엇인지 말하고 특징 2개를 골라서 설명해주세요.',
     '객체지향 프로그래밍의 특징으로는 데이터 보호를 위한 캡슐화, 기능을 단순화하는 추상화, 코드 재사용성을 높이는 상속, 다양한 객체의 유연한 동작을 가능하게 하는 다형성이 있습니다.',
     '', 1, now(), now()),
    ('Stack vs Queue', '스택(Stack)과 큐(Queue)의 작동 방식과 주요 차이점을 설명해주세요.',
